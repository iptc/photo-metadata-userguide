...
=== Image Accessibility for Assistive Technology

Disabled people, such as those who are blind or have low vision, use assistive technologies such as screen readers to navigate text and image content on the web. If a textual description is not provided for an image, assistive technologies will skip over the image as if it doesn’t exist on the page. Text alternatives are required to meet Web Content Accessibility Guidelines (WCAG) Success Criterion 1.1.1 for Non-Text Content.
