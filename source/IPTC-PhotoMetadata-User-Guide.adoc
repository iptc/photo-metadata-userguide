= image:iptc_sv_gradient_70x70.jpg[IPTC logo] Photo Metadata User Guide (DRAFT)
...
|<<General Image Content,Alt Text (Accessibility)>> ||Core
...
|*Caption*|<<General Image Content,Description>>|
...
|<<General Image Content,Description>> (of the full image) ||Core
...
|<<General Image Content,Extended Description (Accessibility)>> ||Core
...
|<<General Image Content,Keywords>> ||Core
...
|<<Rights Information,Source (Supply Chain)>>||Core
...
|<<General Image Content,IPTC Subject Code (Legacy)>> ||Core
...
As this vocabulary is not maintained by IPTC since 2010 the use of this field is a legacy. For the classification of images use the IPTC Media Topics vocabulary now, see: http://cv.iptc.org/newscodes/mediatopic. For the Media Topics the CV Term About Image property must be used - see it just below.
...
This property should not be confused with the IPTC field Headline, which is a brief synopsis or summary of the contents of the image. +
...
The Extended Description (Accessibility) field can be used to provide a more detailed textual description of the purpose and meaning of an image that elaborates on the information provided by the Alt Text (Accessibility) property. Extended Description (Accessibility)  is not required if the Alt Text (Accessibility) field provides a text alternative that serves the equivalent purpose. 
